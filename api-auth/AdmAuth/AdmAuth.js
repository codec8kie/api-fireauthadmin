import { _Fetch, setCookie } from "./AdmAuthFetches";
...
class FireAuthAdm {
  ...
  async getData(api_url) {
    try {
      ...
      this.updatePW = obj ? obj.adm_updatepw_adm : null;
    } catch (error) {
      console.error(error);
    }
  }
   ...
   *************************************************/
  async checkId(id) {
    console.log(this.CHECK_EMAIL, id);
